...
function isCreditCardNumber(ccn) {
  const ccnArray = Array.from(String(ccn), Number);
...
function getCommonDirectoryPath(pathes) {
  ...
  const minLength = Math.min(...splitedArr.map((segment) => segment.length));
  let commonPath = '';

  for (let i = 0; i < minLength; i += 1) {
    const currentSegment = splitedArr[0][i];
    if (splitedArr.every((segment) => segment[i] === currentSegment)) {
